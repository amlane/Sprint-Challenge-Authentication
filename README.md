...
1. What is the purpose of using _sessions_?
   Sessions allow us a way to persist data across requests. Without sessions we would have to re-enter credentials on every new HTTP request you make to the server that requires authorization.

1. What does bcrypt do to help us store passwords in a secure manner.
   Bcryptjs hashes the password for users so even the database owner cannot see what the true password is in the database.

1. What does bcrypt do to slow down attackers?
   Bcrypt uses a security algorithm called a Key Derivation Function. This type of cryptographic hashing algrithm is optimized for security over speed.
